...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component convert_10x_h5_to_h5ad")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "convert_10x_h5_to_h5ad." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process convert_10x_h5_to_h5ad_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.convert_10x_h5_to_h5ad.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow convert_10x_h5_to_h5ad {
  ...
  def key = "convert_10x_h5_to_h5ad"
  ...
  result_ = convert_10x_h5_to_h5ad_process(id_input_output_function_cli_params_)
...
workflow {
  def id = params.id
  def fname = "convert_10x_h5_to_h5ad"
  ...
  def inputFiles = params.convert_10x_h5_to_h5ad
    .arguments
  ...
  result = convert_10x_h5_to_h5ad(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.convert_10x_h5_to_h5ad.output = "convert_10x_h5_to_h5ad.log"

  Channel.from(rootDir) \
    | filter { params.convert_10x_h5_to_h5ad.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.convert_10x_h5_to_h5ad.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | convert_10x_h5_to_h5ad

  emit:
  convert_10x_h5_to_h5ad.out
}
...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_basespace")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "download_basespace." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process download_basespace_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.download_basespace.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow download_basespace {
  ...
  result_ = download_basespace_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.download_basespace
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.download_basespace.output = "download_basespace.log"

  Channel.from(rootDir) \
    | filter { params.download_basespace.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_basespace.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_basespace

  emit:
  download_basespace.out
}
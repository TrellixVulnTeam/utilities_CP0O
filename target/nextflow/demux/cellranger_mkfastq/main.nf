...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component cellranger_mkfastq")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "cellranger_mkfastq." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process cellranger_mkfastq_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.cellranger_mkfastq.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow cellranger_mkfastq {
  ...
  result_ = cellranger_mkfastq_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.cellranger_mkfastq
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.cellranger_mkfastq.output = "cellranger_mkfastq.log"

  Channel.from(rootDir) \
    | filter { params.cellranger_mkfastq.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.cellranger_mkfastq.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | cellranger_mkfastq

  emit:
  cellranger_mkfastq.out
}
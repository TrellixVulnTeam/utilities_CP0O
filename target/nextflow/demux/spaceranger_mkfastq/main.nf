...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component spaceranger_mkfastq")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "spaceranger_mkfastq." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process spaceranger_mkfastq_process {
  tag "${id}"
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.spaceranger_mkfastq.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow spaceranger_mkfastq {
  ...
  result_ = spaceranger_mkfastq_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.spaceranger_mkfastq
    .arguments
  ...
  result = spaceranger_mkfastq(ch_)
  result.view{ it[1] }
}
...
workflow test {
  ...
  main:
  params.test = true
  params.spaceranger_mkfastq.output = "spaceranger_mkfastq.log"

  Channel.from(rootDir) \
    | filter { params.spaceranger_mkfastq.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.spaceranger_mkfastq.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | spaceranger_mkfastq

  emit:
  spaceranger_mkfastq.out
}